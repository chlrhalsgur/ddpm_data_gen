...
class DiffusionUNetLegacy(nn.Module):
    ...
    def forward1(self, x, timesteps, mask=None):
        """ 
        :param x: [B x n_in_channels x ...]
        :param timesteps: [B]
        :param mask: Optional [B x 1 x H x W] mask tensor
        :return: (restored image, predicted noise), same shape as x
        """
        # time embedding
        emb = self.time_embed(
            timestep_embedding(timesteps, self.n_base_channels)
        ...
        if self.skip_connection:
            restored_image = self.proj(h) + x # [B x n_in_channels x ...]
        else:
            restored_image = self.proj(h) # [B x n_in_channels x ...]
        
        # Noise prediction
        noise_pred = restored_image  # Assuming the model predicts the noise here
        
        if mask is not None:
            masked_output = restored_image * mask + x * (1 - mask)
        else:
            masked_output = restored_image
        
        return masked_output, noise_pred



    def forward3(self, x, timesteps, mask=None):
        """ 
        :param x: [B x n_in_channels x ...]
        :param timesteps: [B]
        :param mask: Optional [B x 1 x H x W] mask tensor
